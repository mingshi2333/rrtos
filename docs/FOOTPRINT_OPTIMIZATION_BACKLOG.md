...
- Supported-lane evidence comes from `build/apps/mnist_app/mnist_validation_footprint.md` and `build-be_u1000/rrtos_be_u1000_footprint.md`.
...
| `M2` | Right-size AI arena and model buffers on the RV32 AI lane | moderate RAM recovery on `mnist_app` | medium | canonical AI path profiling and regression checks |
...
| `D1` | Major AI runtime memory model changes | cross-cutting and outside the current board-readiness focus | canonical AI path needs separate approved change and acceptance gates |
