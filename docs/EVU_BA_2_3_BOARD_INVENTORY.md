...
This note corrects the earlier `BD2` framing. The current `be_u1000` board path in this repo already targets the `BE-U1000` SoC on the `EVU-BA` evaluation board family and is materially aligned with the BE-EVB-U1000 / EVU-BA-2.3 instruction v2.3.1.
...
- `docs/EVU_BA_PINMAP.md` now cross-checks the official EVU-BA-2.3 connector tables and matches the active board pinmux groups for `UART0`, `I2C0`, `SPI1`, `QSPI1`, `CANFD0`, `CANFD1`, user LED, and user button.
...
- High confidence: `UART0`, `I2C0`, `SPI1`, `QSPI1`, `CANFD0`, `CANFD1`, user LED, and user button match the current EVU-BA-2.3 documentation and repo pinmux groups.
...
- Lower confidence: runtime closure for `CANFD0`, `CANFD1`, and `QSPI1` is still simulation-heavy rather than silicon-backed.
